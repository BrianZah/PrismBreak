...
namespace pass{
  ...
  void Bookmarks::init(const std::shared_ptr<Context>& context,
                       ...
                       const float& prismTilePosZ)
  {
    ...
    std::cout << "[Info] (Bookmarks::attributeToFontNumber) ";
    ...
    if(fontArr) {
      ...
      std::cout << "[Info] (Bookmarks::init) width, height, channels = " << width << ", " << height << ", " << channels << std::endl;
  ...
  void Bookmarks::execute() const {
      ...
      //glClear(GL_DEPTH_BUFFER_BIT);
        ...
        //mSwap = (mSwap+1)%2;
        //if(mSwap==1) {
        //  mSum.bindImage(2, GL_READ_ONLY);
        //  mSumSwap.bindImage(3, GL_WRITE_ONLY);
        //} else {
        //  mSumSwap.bindImage(2, GL_READ_ONLY);
        //   mSum.bindImage(3, GL_WRITE_ONLY);
        // }
        ...
        // updateMetricsIf(mUpdateMetrics);
        mWireShader.use();
  ...
  const Context::Model& Bookmarks::getModel() const {return mModel;}
  void Bookmarks::setModel(const Context::Model& model, const bool& update) {
