...
namespace pass{
  ...
  void RayCast::init(const std::shared_ptr<const Context>& context,
                     ...
                     const gl::Texture<GL_TEXTURE_2D>* points)
  {
    ...
    mAlbedo = gl::Texture<GL_TEXTURE_2D>(GL_RGBA16F, {mPoints->size(0), mPoints->size(1)}, GL_NEAREST, GL_CLAMP_TO_EDGE);
    mSum = gl::Texture<GL_TEXTURE_1D>(GL_R32F, {mEvalCams.size()+1}, GL_NEAREST, GL_CLAMP_TO_EDGE);
    std::array<int, 2> selected = {-1, -1};
    mSelected = gl::Texture<GL_TEXTURE_1D>(selected, GL_RED_INTEGER, GL_R16I, {2}, GL_NEAREST, GL_CLAMP_TO_EDGE);
    ...
    if(fontArr) {
      ...
      std::cout << "[Info](RayCast::init) width, height, channels = " << width << ", " << height << ", " << channels << std::endl;
  ...
  std::vector<unsigned char> RayCast::attributesToFontDigits(const std::vector<std::string>& attributes, const int& numLetters) const {
    ...
    int j = 0;
    std::cout << "[Info] (RayCast::attributeToFontNumber) ";
  ...
  void RayCast::updateSpecs() {
    std::cout << "[Info](RayCast::updateSpecs)" << std::endl;
    mShader.setSpecs({gl::Shader::Specs(
      GL_COMPUTE_SHADER,
  ...
  void RayCast::setContext(const std::shared_ptr<const Context>& context) {
    mContext = context;
  }
  ...
  void RayCast::setFixUniforms() const {
    ...
    std::cout << "dimensions = " << mContext->dimensions() << std::endl;
    for(int i = 0; i < mContext->numDistributions(mModel); ++i) {
      std::cout << "height_alt[" << i << "] = " << hight[i] << std::endl;
      std::cout << "scaleInv_alt[" << i << "] = " << std::endl;
      for(int j = 0; j < 9; ++j) std::cout << scaleInv[i*9 + j] << (j%3 != 2 ? ", " : "\n");
    }
  ...
  void RayCast::execute() const {
    ...
    //std::cout << glm::to_string(mCamera->getPosition()) << std::endl;
    ...
    for(int i = 0; i < mEvalCams.size(); ++i) {
      ...
      //std::cout << glm::to_string(mEvalCams[i].position) << std::endl;
    ...
    //for(int i = 0; i < mContext->numDistributions(mModel); ++i) {
      //int index = i*(mEvalCams.size()+1);
      //mShader.setFloat("factor[" + std::to_string(i) + "]" + "[" + std::to_string(0) + "]",
      //                 mContext->camPosTMinusMeanT_ScaleInv_camPosMinusMean(mCamera->getPosition(), i));
  ...
  std::array<int, 2> RayCast::getSelectedDistributionAndPoint() {
    if(-1 == mCursor.x) return {-1, -1};
      ...
      std::cout << gl::getAllErrors() << std::endl;
  ...
  const Context::Model& RayCast::getModel() const {return mModel;}
  void RayCast::setModel(const Context::Model& model, const bool& update) {
  ...
  const gl::Texture<GL_TEXTURE_2D>& RayCast::getAlbedo() const {return mAlbedo;}
