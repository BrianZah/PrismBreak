...
namespace pass{
  ...
  void Prism::init(const std::shared_ptr<Context>& context,
                   ...
                   const std::shared_ptr<vis::Camera>& cameraTile)
  {
    ...
    std::cout << "hexagonalPrism =\n";
    for(int i = 0; i < hexagonalPrism.size(); ++i)
      std::cout << hexagonalPrism[i] << ((i)%3 != 2 ? ", " : "\n");
    ...
    for(int i = 0; i < vertices.size(); ++i)
      std::cout << vertices[i] << ((i)%4 != 3 ? ", " : "\n");

    std::array<int, 3> selected = {-1, -1, -1};
    mUIElement = gl::Texture<GL_TEXTURE_1D>(selected, GL_RED_INTEGER, GL_R8I, {3}, GL_NEAREST, GL_CLAMP_TO_EDGE);

    mTokenLenght = 6;
    auto attributes = mContext->getAttributes();
    std::vector<unsigned char> fontDigits = attributesToFontDigits(attributes, mTokenLenght);
    mAttributes = gl::Texture<GL_TEXTURE_1D>(fontDigits, GL_RED_INTEGER, GL_R8UI, {mTokenLenght*mContext->numComponents(0)}, GL_NEAREST, GL_CLAMP_TO_EDGE);
    std::cout << "[Info] (Prism::attributeToFontNumber) ";
    for(const auto& digits : fontDigits) std::cout << +digits << ", ";
    std::cout << std::endl;
    ...
    if(fontArr) {
      ...
      std::cout << "[Info](Prism::init) width, height, channels = " << width << ", " << height << ", " << channels << std::endl;
      mFont = gl::Texture<GL_TEXTURE_2D>(font, GL_RGBA, GL_RGBA8, {width, height}, GL_LINEAR, GL_CLAMP_TO_EDGE);
      stbi_image_free(fontArr);
    } else {std::cout << "[Error](Prism::init) " << path << " not found" << std::endl;}

    std::cout << "[Info](Prism::init) test0" << std::endl;
    mScaleByWeight = true;
    mThreshold = 2.0f;
    ...
    mColorsStudent.resize(mContext->numDistributions(Context::tmm)*3);
    for(int i = 0; i < mColorsStudent.size(); ++i) mColorsStudent[i] = colors[i%colors.size()];
    mColorsGaussian.resize(mContext->numDistributions(Context::gmm)*3);
    ...
    mWireShader.setSpecs({
      gl::Shader::Specs(GL_VERTEX_SHADER, shaderpath + "wire.vs"),
      gl::Shader::Specs(GL_GEOMETRY_SHADER, shaderpath + "wire.gs"),
      gl::Shader::Specs(GL_FRAGMENT_SHADER, shaderpath + "wire.fs")
    });
    mWireShader.refresh();
    mWireShader.use();
  ...
  std::vector<unsigned char> Prism::attributesToFontDigits(const std::vector<std::string>& attributes, const int& numLetters) const {
    ...
    for(const auto& attribute : attributes) {
      for(int i = 0; i < numLetters; ++i) {
        ...
        std::cout << attribute[i] << "->" << +fontDigits[j-1] << ", ";
      }
    }
    std::cout << std::endl;
    return fontDigits;
  ...
  void Prism::updateSpecs() {
    std::cout << "[Info](Prism::updateSpecs) level = " << mStage << std::endl;
    std::string shaderpath = std::string(CMAKE_SHADERDIR) + "prism/level" + std::to_string(mStage);
    mShader.setSpecs({
      gl::Shader::Specs(GL_VERTEX_SHADER, shaderpath + ".vs"),
      gl::Shader::Specs(GL_FRAGMENT_SHADER, shaderpath + ".fs",
  ...
  void Prism::prepareBuffersAndTextures() {
    double time0, time1, time2, time3, time4, time5, time6, time7, time8, time9, time10, time11;
    auto start = std::chrono::high_resolution_clock::now();
    auto modelParameters = mContext->getParametersStd140(mModel, mStage);
    auto end = std::chrono::high_resolution_clock::now();
    time0 = std::chrono::duration_cast<std::chrono::duration<double>>(end - start).count();
    std::cout << "[Info] (Prism::prepareBuffersAndTextures) mContext->getParametersStd140(mModel, mStage);" << std::endl;

    std::cout << "[Info](Prism::prepareBuffersAndTextures) modelParameters.size() = " << modelParameters.size() << std::endl;
    ...
    time1 = std::chrono::duration_cast<std::chrono::duration<double>>(end - start).count();

    const int numComponentsSets = mContext->numComponentsSets(mStage);
    const int numComponents = mContext->numComponents(mStage);
    std::cout << "[Info](Prism::prepareBuffersAndTextures) numComponentsSets = " << numComponentsSets << std::endl;
    std::cout << "[Info](Prism::prepareBuffersAndTextures) numComponents = " << numComponents << std::endl;

    start = std::chrono::high_resolution_clock::now();
    auto bookmarks = mBookmarksContext->getIndices(mStage, mModel);
    mBookmarks = gl::Texture<GL_TEXTURE_1D>(bookmarks, GL_RED_INTEGER, GL_R8I, {numComponents*numComponentsSets}, GL_NEAREST, GL_CLAMP_TO_EDGE);
    end = std::chrono::high_resolution_clock::now();
    time2 = std::chrono::duration_cast<std::chrono::duration<double>>(end - start).count();

    start = std::chrono::high_resolution_clock::now();
    std::vector<float> components = mContext->getComponents(mStage);
    mComponents = gl::Texture<GL_TEXTURE_2D>(components, GL_RED, GL_R32F, {mContext->dimensions(), numComponents*numComponentsSets}, GL_NEAREST, GL_CLAMP_TO_EDGE);
    end = std::chrono::high_resolution_clock::now();
    time3 = std::chrono::duration_cast<std::chrono::duration<double>>(end - start).count();
    ...
    time4 = std::chrono::duration_cast<std::chrono::duration<double>>(end - start).count();
    ...
    time5 = std::chrono::duration_cast<std::chrono::duration<double>>(end - start).count();

    start = std::chrono::high_resolution_clock::now();
    mNumMetrics = 3+int(mStage == 2);
    mMetrics = gl::Texture<GL_TEXTURE_2D>(GL_R32F, {numComponents*numComponentsSets, mNumMetrics}, GL_NEAREST, GL_CLAMP_TO_EDGE);
    ...
    time6 = std::chrono::duration_cast<std::chrono::duration<double>>(end - start).count();

    start = std::chrono::high_resolution_clock::now();
    int numDistributions = mContext->numDistributions(mModel);
    if(2 == mStage) {
      //mMetrics.setSubImage(sparsity, {0, 3}, {numComponents*numComponentsSets, 1});
      mOverlap = gl::Texture<GL_TEXTURE_3D>(GL_R32F, {numDistributions*numDistributions, numComponents, numComponentsSets}, GL_NEAREST, GL_CLAMP_TO_EDGE);
      mNumPixels = gl::Texture<GL_TEXTURE_1D>(GL_R32I, {numComponents*numComponentsSets}, GL_NEAREST, GL_CLAMP_TO_EDGE);
    }
    end = std::chrono::high_resolution_clock::now();
    time7 = std::chrono::duration_cast<std::chrono::duration<double>>(end - start).count();

    start = std::chrono::high_resolution_clock::now();
    mUpdateVisibilityMetric = false;
    mOverlapNew = gl::Texture<GL_TEXTURE_2D>(GL_R32F, {numDistributions*numDistributions, numComponents*numComponentsSets}, GL_NEAREST, GL_CLAMP_TO_EDGE);
    updateVisibility();
    end = std::chrono::high_resolution_clock::now();
    time8 = std::chrono::duration_cast<std::chrono::duration<double>>(end - start).count();

    start = std::chrono::high_resolution_clock::now();
    sortTiles(mMetric);
    end = std::chrono::high_resolution_clock::now();
    time9 = std::chrono::duration_cast<std::chrono::duration<double>>(end - start).count();

    std::cout << "[Info] (Prism::prepareBuffersAndTextures) mContext->getParametersStd140(mModel, mStage);" << std::endl;
    std::cout << "tim0: " << time0 << std::endl;
    std::cout << "tim1: " << time1 << std::endl;
    std::cout << "tim2: " << time2 << std::endl;
    std::cout << "tim3: " << time3 << std::endl;
    std::cout << "tim4: " << time4 << std::endl;
    std::cout << "tim5: " << time5 << std::endl;
    std::cout << "tim6: " << time6 << std::endl;
    std::cout << "tim7: " << time7 << std::endl;
    std::cout << "tim8: " << time8 << std::endl;
    std::cout << "tim9: " << time9 << std::endl;
  ...
  void Prism::setFixUniforms() const {
    ...
    auto& colors = Context::tmm == mModel ? mColorsStudent : mColorsGaussian;
...
/*
  void Prism::updateVisibilityMetric() const {
    return;
    if(0 == mStage) return;
    if(1 == mStage && not mUpdateVisibilityMetric) return;
    if(2 == mStage && not mUpdateVisibilityMetric) {
    ...
    mVisibility.get();
    mUpdateVisibilityMetric = false;
    mShader.setInt("updateMetics", mUpdateVisibilityMetric);
    auto evalData = mOverlap.getImage<Eigen::VectorXf>();
    mOverlap.clearImage();
    int numDistributions = mContext->numDistributions(mModel);
    int numComponents = mContext->numComponents(mStage);
    int numComponentsSets = mContext->numComponentsSets(mStage);
    std::cout << "evalData.size()/(numDistributions*numDistributions) = " << evalData.size()/(numDistributions*numDistributions) << std::endl;
    int cnt = 0;
    std::vector<float> sumSoftMax = getSumSoftMax(evalData);
    for(int window = 0; window < sumSoftMax.size(); ++window) {
      std::cout << "sumSoftMax[" << window << "] = " << sumSoftMax[window] << std::endl;
    }

    for(const auto& elem : evalData) std::cout << elem << (++cnt%9 == 0 ? "\n" : cnt%3 == 0 ? " | " : " ");
    for(const auto& elem : sumSoftMax) std::cout << elem << (++cnt%numComponents == 0 ? "\n" : " ");
    ...
    for(int window = 0; window < mSoftMaxIndices.size(); ++window) {
      std::cout << "sumSoftMax[" << window << "] = " << mSoftMaxIndices[window] << std::endl;
    }
    mMetrics.setSubImage(sumSoftMax, {0, 2}, {numComponents*numComponentsSets, 1});
    if(visibility == mMetric)
      mIndexArray = gl::Texture<GL_TEXTURE_1D>(mSoftMaxIndices, GL_RED_INTEGER, GL_R16I, {mSoftMaxIndices.size()}, GL_NEAREST, GL_CLAMP_TO_EDGE);
  }
*/
  void Prism::updateVisibility() const {
    ...
    int numComponents = mContext->numComponents(mStage);
    int numComponentsSets = mContext->numComponentsSets(mStage);
    std::cout << "mOverlapNew.size() = " << mOverlapNew.size() << std::endl;;
    std::cout << "evalData.size()/(numDistributions*numDistributions) = " << evalData.size()/(numDistributions*numDistributions) << std::endl;
    int cnt = 0;
    for(const auto& elem : evalData) std::cout << elem << ((++cnt)%9 == 0 ? "\n" : ", " );

    std::vector<float> sumSoftMax = getSumSoftMax(evalData, 256*256);
    for(int window = 0; window < sumSoftMax.size(); ++window) {
      std::cout << "sumSoftMax[" << window << "] = " << sumSoftMax[window] << std::endl;
    }
  ...
  void Prism::sortTiles(const Metric& metric) {
    switch(metric) {
      case variance: {
        mIndexArray = gl::Texture<GL_TEXTURE_1D>(mEigenvalueIndices, GL_RED_INTEGER, GL_R16I, {mEigenvalueIndices.size()}, GL_NEAREST, GL_CLAMP_TO_EDGE);
        break;
      }
      case sparsity: {
        mIndexArray = gl::Texture<GL_TEXTURE_1D>(mSparsityIndices, GL_RED_INTEGER, GL_R16I, {mSparsityIndices.size()}, GL_NEAREST, GL_CLAMP_TO_EDGE);
        break;
      }
      case visibility: {
        //if(1 <= mStage) {
          //mShader.use();
          //mUpdateVisibilityMetric = true;
          //mShader.setInt("updateMetics", mUpdateVisibilityMetric);
          updateVisibility();
          mIndexArray = gl::Texture<GL_TEXTURE_1D>(mSoftMaxIndices, GL_RED_INTEGER, GL_R16I, {mSparsityIndices.size()}, GL_NEAREST, GL_CLAMP_TO_EDGE);
        //}
        break;
      }
    }
  ...
  const Context::Model& Prism::getModel() const {return mModel;}
  void Prism::setModel(const Context::Model& model, const bool& update) {
  ...
  std::array<float, 3> Prism::getColor(const int& i) const {
    return Context::tmm == mModel ? std::array<float, 3>{mColorsStudent[i*3], mColorsStudent[i*3+1], mColorsStudent[i*3+2]}
  ...
  void Prism::execute() const {
      ...
      glDepthFunc(GL_LESS);
        ...
        //std::cout << "[Info] (Prism::execute) selectedTiles = ";
        //for(const auto elem : mContext->getIndexSetsAndComps(mStage)) std::cout << elem << ", ";
        //std::cout << std::endl;
        ...
        mUIElement.bindImage(7, GL_WRITE_ONLY);
  ...
  void Prism::updateBookmarks() {
    ...
    int numComponentsSets = mContext->numComponentsSets(mStage);
    auto bookmarks = mBookmarksContext->getIndices(mStage, mModel);
    std::cout << "[Info](Prism::updateBookmarks) mIndexArray =" << std::endl;
    int i = 0;
    for(const auto& elem : bookmarks) std::cout << elem << (++i%numComponents == 0 ? "\n" : ", ");
    std::cout << std::endl;
    mBookmarks = gl::Texture<GL_TEXTURE_1D>(bookmarks, GL_RED_INTEGER, GL_R8I, {numComponents*numComponentsSets}, GL_NEAREST, GL_CLAMP_TO_EDGE);
  }
