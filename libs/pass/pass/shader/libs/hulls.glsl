...
float hulls[numHulls] = float[numHulls]((userInput.adoptToStd ? 0.5f : 5.0f)*userInput.threshold, (userInput.adoptToStd ? 1.0f : 1.0f)*userInput.threshold);
//float hulls[numHulls] = float[numHulls](1.0f, 2.0f);
...
/*
float densityStandardDerivation(int iDis, float mahalanobisDistance) {
  float innerProduct = mahalanobisDistance*mahalanobisDistance;
  if(mixtureModel == gmm) {
    return height(iDis) * exp(-0.5f*innerProduct);
    } else {
      return height(iDis) * pow(1.0f+innerProduct/(df(iDis)-2.0f), -0.5f*(df(iDis)+3) );
    }
  }
float getIsoValue(int iDis, int iHull) {
  if(mixtureModel == tmm)
    return 1.0f*df(iDis)*(pow(densityStandardDerivation(iDis, hulls[iHull])/height(iDis), -2.0f/(df(iDis) + 3.0f)) - 1.0f);
  else if(mixtureModel == gmm)
    return -2.0f*log(densityStandardDerivation(iDis, hulls[iHull])/height(iDis));
}
*/

float squaredMahalanobisOfHull(int iDis, int iHull) {
  if(userInput.adoptToStd) return hulls[iHull]*hulls[iHull];
  if(mixtureModel == tmm)
...
vec2 getDeltasIntersectionPoints(int iDis, int iHull, Ray ray, float t) {
  float iso_gauss = squaredMahalanobisOfHull(iDis, iHull);
...
Hull getNextHull(Hull hullLast, Ray ray, ivec2 iDisRange) {
  ...
  for(int iDis = iDisRange.x; iDis < iDisRange.y; iDis++) {
    if(colors[iDis%numDistributions].r*colors[iDis%numDistributions].g*colors[iDis%numDistributions].b > 0.99f) continue;
...
vec4 rayCastProcedure(Ray ray, int depth, ivec2 q, float tBorder, vec3 lightPos, ivec2 iDisRange) {
  ...
  for (int i = 0; i < depth; i++) {
    ...
    vec3 colorOuterHull = rgb2hsv(colors[hull.iDis%numDistributions]);
    colorOuterHull.z = 0.0;
    colorOuterHull = hsv2rgb(colorOuterHull);
    vec3 colorInnerHull = rgb2hsv(colors[hull.iDis%numDistributions]);
...
vec4 rayCastProcedure(Ray ray, int depth, ivec2 q, vec3 lightPos) {
  float tBorder = 1000.0f;
  return rayCastProcedure(ray, depth, q, tBorder, lightPos, ivec2(0, numDistributions));
}
