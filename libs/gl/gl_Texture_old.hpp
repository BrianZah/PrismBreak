...
namespace gl {
  ...
  public:
    ...
    constexpr Texture<mGlTexture>& operator=(Texture&& other);
  ...
  template<GLenum mGlTexture>
  inline void Texture<mGlTexture>::create() {
  ...
  template<GLenum mGlTexture>
  inline void Texture<mGlTexture>::discard() {glDeleteTextures(1, &mId);}

  template<GLenum mGlTexture>
  inline Texture<mGlTexture>::Texture()
  ...
  inline Texture<mGlTexture>::Texture(const GLenum& internalformat,
                             const GLsizei (&sizes) [ns],
                             const GLenum (&filterRules) [nf],
                             const GLenum (&borderBehaveiors) [nb],
                             const GLint& levels,
  ...
  inline Texture<mGlTexture>::Texture(const A& image, const GLenum& format,
                                      const GLenum& internalformat,
                                      const GLsizei (&sizes) [ns],
                                      const GLenum (&filterRules) [nf],
                                      const GLenum (&borderBehaveiors) [nb],
  ...
  template<GLenum mGlTexture>
  constexpr Texture<mGlTexture>& Texture<mGlTexture>::operator=(Texture&& other) {
  ...
  template<GLenum mGlTexture>
  template<int n>
  inline void Texture<mGlTexture>::resizeAndClear(const GLsizei (&sizes) [n], const GLint& levels, const GLenum& internalformat) {
    mSizes = std::to_array(sizes);
    mLevels = levels;
    mInternalformat = internalformat;
    create();
  ...
  template<GLenum mGlTexture>
  template<int n>
  inline void Texture<mGlTexture>::resizeAndClear(const GLsizei (&sizes) [n], const GLint& levels) {
    mSizes = std::to_array(sizes);
    mLevels = levels;
    create();
  }
  ...
  template<GLenum mGlTexture>
  template<int n>
  inline void Texture<mGlTexture>::resizeAndClear(const GLsizei (&sizes) [n]) {
    mSizes = std::to_array(sizes);
    create();
  }
  ...
  template<GLenum mGlTexture>
  template<int n>
  constexpr void Texture<mGlTexture>::setFilterRules(const GLenum (&filterRules) [n]) {
    mFilterRules = std::to_array(filterRules);
    glTextureParameteri(mId, GL_TEXTURE_MIN_FILTER, mFilterRules[0]);
    glTextureParameteri(mId, GL_TEXTURE_MAG_FILTER, mFilterRules[1]);
    if(mGenerateMipmap) glGenerateTextureMipmap(mId);
  ...
  template<GLenum mGlTexture>
  constexpr void Texture<mGlTexture>::setFilterRules(const GLenum& filterRules) {
    setFilterRules({filterRules, filterRules});
  }

  template<GLenum mGlTexture>
  constexpr void Texture<mGlTexture>::generateMipmap(const bool& generateMipmap) {
    mGenerateMipmap = generateMipmap;
    if(mGenerateMipmap) glGenerateTextureMipmap(mId);
  }
  ...
  template<GLenum mGlTexture>
  template<int n>
  constexpr void Texture<mGlTexture>::setBorderBehaveiors(const GLenum (&borderBehaveiors) [n]) {
    mBorderBehaveiors = std::to_array(borderBehaveiors);
    glTextureParameteri(mId, GL_TEXTURE_WRAP_S, mBorderBehaveiors[0]);
    if(mBorderBehaveiors.size() > 1) glTextureParameteri(mId, GL_TEXTURE_WRAP_T, mBorderBehaveiors[1]);
    if(mBorderBehaveiors.size() > 2) glTextureParameteri(mId, GL_TEXTURE_WRAP_R, mBorderBehaveiors[2]);
  ...
  template<GLenum mGlTexture>
  constexpr void Texture<mGlTexture>::setBorderBehaveiors(const GLenum& borderBehaveiors) {
    switch(numBorders()) {
      case 1:
  ...
  template<GLenum mGlTexture>
  template<int n>
  constexpr void Texture<mGlTexture>::setBorderColor(const float (&borderColor) [n]) {
    mBorderColor = std::to_array(borderColor);
    glTextureParameterfv(mId, GL_TEXTURE_BORDER_COLOR, mBorderColor.data());
  }

  template<GLenum mGlTexture>
  inline void Texture<mGlTexture>::bind(unsigned int textureUnit) const {
    glBindTextureUnit(textureUnit, mId);
  }

  template<GLenum mGlTexture>
  inline void Texture<mGlTexture>::bindImage(const GLuint& textureUnit, const GLenum& access, const GLint& level) const {
    glBindImageTexture(textureUnit, mId, level, GL_FALSE, 0, access, mInternalformat);
  }

  template<GLenum mGlTexture>
  constexpr const GLsizei Texture<mGlTexture>::size() const {
    int prod = 1;
  ...
  template<GLenum mGlTexture>
  constexpr const GLsizei& Texture<mGlTexture>::size(const int& dimension) const {return mSizes[dimension];}

  template<GLenum mGlTexture>
  template<class A>
  constexpr A Texture<mGlTexture>::getImage(const GLint& level, const GLenum& format) const {
    A imageArray;
    imageArray.resize(this->size()*gl::numChannels(format));
    GLenum type = gl::enumType<typename A::value_type>();
  ...
  template<GLenum mGlTexture>
  template<class A>
  constexpr A Texture<mGlTexture>::getImage(const GLint& level) const {
    return getImage<A>(level, gl::baseFormat(mInternalformat));
  }

  template<GLenum mGlTexture>
  template<class A, int ns>
  constexpr A Texture<mGlTexture>::getSubImage(const GLint (&offsets) [ns], const GLsizei (&sizes) [ns], const GLint& level, const GLenum& format) const {
    ...
    A imageArray;
    imageArray.resize(prod*gl::numChannels(format));
    switch(dimensions()) {
      case 1:
  ...
  template<GLenum mGlTexture>
  template<class A, int ns>
  constexpr A Texture<mGlTexture>::getSubImage(const GLint (&offsets) [ns], const GLsizei (&sizes) [ns], const GLint& level) const {
    return getSubImage<A>(offsets, sizes, level, gl::baseFormat(mInternalformat));
  }

  template<GLenum mGlTexture>
  template<class A, int ns>
  constexpr void Texture<mGlTexture>::setSubImage(const A& imageArray, const GLint (&offsets) [ns], const GLsizei (&sizes) [ns], const GLint& level, const GLenum& format) const {
  ...
  template<GLenum mGlTexture>
  template<class A, int ns>
  constexpr void Texture<mGlTexture>::setSubImage(const A& imageArray, const GLint (&offsets) [ns], const GLsizei (&sizes) [ns], const GLint& level) const {
    setSubImage<A>(imageArray, offsets, sizes, level, gl::baseInternalFormat(mInternalformat));
  }

  template<GLenum mGlTexture>
  template<class A>
  constexpr void Texture<mGlTexture>::setImage(const A& imageArray, const GLint& level, const GLenum& format) const {
    if(mLevels <= level) throw std::string("[Error]");
  ...
  template<GLenum mGlTexture>
  template<class A>
  constexpr void Texture<mGlTexture>::setImage(const A& imageArray, const GLint& level) const {
    setImage<A>(imageArray, level, gl::baseInternalFormat(mInternalformat));
  }

  template<GLenum mGlTexture>
  constexpr void Texture<mGlTexture>::clearImage(const GLint& level) const {
    GLsizei width = mSizes.size() > 0 ? mSizes[0] : 1;
    GLsizei height = mSizes.size() > 1 ? mSizes[1] : 1;
    GLsizei depth = mSizes.size() > 2 ? mSizes[2] : 1;
    GLenum format = baseInternalFormat(mInternalformat);
  ...
  template<GLenum mGlTexture>
  inline Texture<mGlTexture>::~Texture() {discard();}
} // namespace gl
