...
void PrismFramebuffer::init(const int& width, const int& height) {
  mColor = glPP::Texture<GL_TEXTURE_2D>(GL_RGBA16F, {width, height}, GL_NEAREST, GL_CLAMP_TO_EDGE);
  mWindowId = glPP::Texture<GL_TEXTURE_2D>(GL_RG8I, {width, height}, GL_NEAREST, GL_CLAMP_TO_EDGE);
  ...
  mDepth = glPP::Renderbuffer(GL_DEPTH_COMPONENT32F, width, height);
...
const glPP::Texture<GL_TEXTURE_2D>& PrismFramebuffer::getColor() const {
  return mColor;
}
