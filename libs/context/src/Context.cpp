...
{
  std::cout << "[Info](Context::Context) test1" << std::endl;
  try{
...
// <- singularvectors vs. eigenvectors ->
  ...
  auto mean_scaled = mData.rowwise().mean();
  std::cout << "[Info](Context::Context) mUnsacaledMean = " << mUnsacaledMean << std::endl;
  std::cout << "[Info](Context::Context) mean(scaled) = " << mean_scaled << std::endl;

  mUnsacaledStd = (mUnscaledData.array().square().rowwise().mean() - mUnsacaledMean.array().square()).sqrt().matrix();
  auto std_scaled = mData.array().square().matrix().rowwise().mean() - mean_scaled.array().square().matrix();
  std::cout << "[Info](Context::Context) mUnsacaledStd = " << mUnsacaledStd << std::endl;
  std::cout << "[Info](Context::Context) std(scaled) = " << std_scaled << std::endl;
}

const std::string& Context::getDir() const {return mDir;}

int Context::showCanonicalBasis(const Model& model, const int& stage) const {
  switch(stage) {
    case 0 : return 1;
    case 1 : return mISetsIComps(0, 0) == 0;
    case 2 : return mISetsIComps(0, 0) == 0 && mISetsIComps(0, 1) == 0;
    default: return 0;
  }
}
...
std::vector<int> Context::getIndexSetsAndComps(const int& currentStage) const {
  std::vector<int> iSetsIComps;
  iSetsIComps.reserve(currentStage*2);
  for(int stage = 0; stage < currentStage; ++stage)
    iSetsIComps.insert(iSetsIComps.end(), {mISetsIComps(0, stage), mISetsIComps(1, stage)});
  return iSetsIComps;
}
...
void Context::prepare(const Model& model, const int& stage, const int& iCompSet, const int& iComp) {
  ...
  if(3 == stage) return;
  //std::cout << "[Info](Context::prepare) mOrthoSpaceLevel1 = \n" << mOrthoSpaceLevel1 << std::endl;
  //mOrthoSpaceLevel1 = Eigen::MatrixXf(mTMM[0].dimensions(), mTMM[0].dimensions()-1);
...
void Context::extendViewBox(const Model& model, const int& stage, const int& iCompSet, const int& iComp) {
  ...
  mISetsIComps.col(stage-1) << iCompSet, iComp;
  mState[stage-1] = Token(iCompSet <= showedCanonBasis ? Context::nomm : model, iCompSet, iComp);
  std::cout << "[Info](Context::prepare) mISetsIComps = " << mISetsIComps << std::endl;
...
void Context::resizeComponentsAndWeights(const Model& model, const int& stage) {
  ...
  std::cout << "[Info](Context::prepare) showCanonBasis = " << showCanonBasis << std::endl;
